...
class ControladorPID:
    def __init__(self, Kp=2.0, Ti=60.0, Td=5.0, u_min=20.0, u_max=100.0):
        self.Kp = Kp
        self.Ti = max(Ti, 1e-3)
        self.Td = Td
        self.u_min = u_min
    ...
    def calcular(self, setpoint, medido, dt):
        ...
        if u_raw != u_sat and error * u_raw > 0:
            self.integral -= error * dt
...
class VentanaSimulacionDinamica(QDialog):
    ...
    def init_ui(self):
        ...
        self.txt_kp = QLineEdit("3.5")
        self.txt_ti = QLineEdit("120")
        self.txt_td = QLineEdit("10")
    ...
    def examinar_epw(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Seleccionar archivo EPW", "", "Archivos EPW (*.epw)")
        if file_path:
