...
# ==========================================
# 5. HILO DE SIMULACIÓN DINÁMICA CON KPIs EXTENDIDOS
# ==========================================
...
class SimularDinamicaWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # Warm-up de estado estacionario
            tdb_0 = tdb_interp[0]
            twb_0 = twb_interp[0]
            patm_0 = patm_interp[0]
            
            w_sat_wb0 = humedad_saturacion(twb_0, patm_0)
            w_a_in0 = ((h_fg0_def - (cp_w_def - cp_v_def) * twb_0) * w_sat_wb0 - cp_a_def * (tdb_0 - twb_0)) / (h_fg0_def + cp_v_def * twb_0 - cp_w_def * twb_0)
            ...
            for _ in range(30):
                ...
                T_sal_init, _, _, _, _, _ = simular_torre_2d_matriz(
                    NTU_ref, T_w_in_dinamica, m_w_nom, h_a_in0, w_a_in0, m_a_init, patm_0, Nx=6, Ny=6
                )
            ...
            # ACUMULADORES REINICIADOS POST WARM-UP
            ...
            for idx, sec in enumerate(time_steps_sec):
                ...
                w_sat_wb = humedad_saturacion(twb_k, patm_k)
                ...
                agua_acum_m3 += evap_m3h * (dt_sim_sec / 3600.0)

                Q_MWt = (m_w_nom * cp_w_def * delta_T_proceso) / 1000.0
                energia_disipada_mwh_t += Q_MWt * (dt_sim_sec / 3600.0)
            ...
            # CÁLCULO DE KPIS ESTADÍSTICOS Y ENERGÉTICOS
...
class VentanaSimulacionDinamica(QDialog):
    def __init__(self, parent=None, datos_torre=None):
        super().__init__(parent)
        self.setWindowTitle("Simulación Dinámica Anual / Climática (.EPW) con PID, Energía y KPIs")
    ...
    def init_ui(self):
        ...
        self.lbl_int_agua_mwh = QLabel("Consumo Específico Agua: -- m³/MWh_t")
