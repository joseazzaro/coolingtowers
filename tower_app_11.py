...
# ==========================================
# 2. MOTOR POPPE 2D CON EVAPORACIÓN PROTEGIDA (>= 0)
# ==========================================
def simular_torre_2d_matriz(NTU_actual, T_w_in, m_w_total, h_a_in, w_a_in, m_a_total, P_atm=101325.0, Nx=20, Ny=20):
    ...
    for i in range(Ny):      
        for j in range(Nx):  
            ...
            potencial_w = max(0.0, w_sw - w_air_cell)  # Evaporación siempre >= 0
...
class SimularDinamicaWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # Parámetros del motor del ventilador y eficiencia
            p_motor_nom_kw = self.cfg['p_motor_kw']
            eta_glob = max(0.1, min(1.0, self.cfg['eta_fan_pct'] / 100.0))

            # Estado inicial estimado en equilibrio
            T_piscina = T_set + 0.5
            ...
            for idx, sec in enumerate(time_steps_sec):
                ...
                # --- LLAMADA AL PID (Versión que modula la velocidad) ---
                velocidad_pct = pid.calcular(T_set, T_piscina, dt_sim_sec)
                
                # Caudal de aire real modulado por la velocidad del ventilador
                m_a_actual = m_a_nom * (velocidad_pct / 100.0)

                # Consumo de potencia eléctrica instantánea
...
class VentanaSimulacionDinamica(QDialog):
    ...
    def init_ui(self):
        ...
        # ETIQUETAS DE RESULTADOS ACUMULADOS
