...
# ==========================================
# 3. CONTROLADOR PID DISCRETO MEJORADO
# ==========================================
class ControladorPID:
    def __init__(self, Kp=4.0, Ti=300.0, Td=5.0, u_min=20.0, u_max=100.0):
        ...
        self.integral = 0.0
        self.prev_error = None # Se inicializará con el primer error real
    ...
    def calcular(self, setpoint, medido, dt):
        ...
        # Si es el primer ciclo, inicializamos prev_error para evitar saltos (derivative kick)
...
# ==========================================
# 5. HILO DE SIMULACIÓN CON PROTECCIÓN DE MASAS
# ==========================================
    ...
    def run(self):
        try:
            ...
            for _ in range(20):
                ...
                # PROTECCIÓN DE MASA MÍNIMA DE AIRE EN WARM-UP
            ...
            for idx in range(total_pasos):
                ...
                # PROTECCIÓN DE MASA MÍNIMA DE AIRE EN EL BUCLE REAL
...
class VentanaSimulacionDinamica(QDialog):
    def __init__(self, parent=None, datos_torre=None):
        super().__init__(parent)
        self.setWindowTitle("Simulación Dinámica Anual / Climática (.EPW) con PID, Energía y Pre-interpolación")
        ...
        if ax_top is not None:
            if self.chk_niebla.isChecked() and 'niebla' in self.res_sim:
                ...
                if np.any(niebla_mask):
                    # Encontrar los rangos contiguos de inicio y fin de niebla
                    ...
                    # Dibujar rectángulos sombreados limpios
                    for idx, (i, f) in enumerate(zip(inicios, fines)):
                        lbl = 'Pluma/Niebla Activa' if idx == 0 else "" # Solo la primera franja lleva etiqueta
                        ax_top.axvspan(times[i], times[f], color='#7F8C8D', alpha=0.20, linewidth=0, label=lbl)
