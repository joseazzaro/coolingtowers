...
class SimularDinamicaWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # --- INERCIA REAL DEL ESTANQUE Y RETORNO DE PROCESO ---
            ...
            for idx, sec in enumerate(time_steps_sec):
                ...
                T_salida_inst, evap_kg_s = simular_torre_2d_directo(
                    NTU_ref, T_w_in_dinamica, m_w_nom, tdb_k, twb_k, m_a_actual, patm_k,
                    Nx=10, Ny=10
                )
...
class VentanaSimulacionDinamica(QDialog):
    ...
    def init_ui(self):
        ...
        # 2. Rango Temporal, Paso y Volumen
...
class TorreCoolingApp(QMainWindow):
    ...
    def lanzar_worker(self, datos_p1, datos_p2=None):
        """Lanza el hilo de calibración interno del propio archivo."""
        ...
        # Usar directamente CalibracionWorker nativo
