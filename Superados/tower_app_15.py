...
# ==========================================
class PsicroLUT:
    def __init__(self, T_min=-10.0, T_max=60.0, step=0.1, P_atm=101325.0):
        self.T_min = T_min
        self.T_max = T_max
        self.step = step
        self.P_atm = P_atm
...
# ==========================================
# 5. HILO DE SIMULACIÓN DINÁMICA CON ACCELERACIÓN LUT
# ==========================================
...
class SimularDinamicaWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.progreso_signal.emit("Construyendo Tabla Psicrométrica de Ultra-Alta Velocidad (LUT)...", 8)
            ...
            # Warm-up rápido de 20 iteraciones
            tdb_0 = tdb_interp[0]
            twb_0 = twb_interp[0]
            ...
            for idx, sec in enumerate(time_steps_sec):
                ...
                w_a_in = ((h_fg0_def - (cp_w_def - cp_v_def) * twb_k) * w_sat_wb - cp_a_def * (tdb_k - twb_k)) / (h_fg0_def + cp_v_def * tdb_k - cp_w_def * twb_k)
