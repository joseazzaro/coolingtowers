...
# ==========================================
# 1.B TABLA DE BÚSQUEDA RÁPIDA (LOOK-UP TABLE / LUT)
# ==========================================
class PsicroLUT:
...
class ControladorPID:
    def __init__(self, Kp=4.0, Ti=300.0, Td=5.0, u_min=20.0, u_max=100.0):
        self.Kp = float(Kp)
...
# ==========================================
# 5. HILO DE SIMULACIÓN DINÁMICA CON SOLVER DE PASO ADAPTATIVO
# ==========================================
...
class SimularDinamicaWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.progreso_signal.emit("Inicializando Tabla Psicrométrica Fast-LUT...", 8)
            ...
            # Warm-up rápido inicial
            tdb_0 = tdb_epw[0]
            twb_0 = twb_epw[0]
            patm_0 = patm_epw[0]
            
            w_sat_wb0, _ = lut.get_ws_hs(twb_0)
            w_a_in0 = ((h_fg0_def - (cp_w_def - cp_v_def) * twb_0) * w_sat_wb0 - cp_a_def * (tdb_0 - twb_0)) / (h_fg0_def + cp_v_def * tdb_0 - cp_w_def * twb_0)
            h_a_in0 = cp_a_def * tdb_0 + w_a_in0 * (h_fg0_def + cp_v_def * tdb_0)

            T_piscina = max(twb_0 + 1.0, T_set)
            T_w_in_dinamica = T_piscina + delta_T_proceso

            for _ in range(20):
                u_init = pid.calcular(T_set, T_piscina, 300.0)
                m_a_init = m_a_nom * (u_init / 100.0)
                ...
                T_piscina = T_piscina + min(1.0, 300.0 / tau_sec) * (T_sal_init - T_piscina)
                T_w_in_dinamica = T_piscina + delta_T_proceso
            ...
            # LÓGICA DEL SOLVER DE PASO ADAPTATIVO
            t_actual_sec = 0.0
            dt_adapt = 300.0
            dt_min = 60.0
            dt_max = 1200.0
            ...
            u_prev = 50.0
            T_piscina_prev = T_piscina

            while t_actual_sec <= t_total_sec:
                if self._is_cancelled:
                    self.cancelado_signal.emit()
                    return

                pct = int(10 + (t_actual_sec / t_total_sec) * 85)
                if len(times) % 50 == 0:
                    dt_fecha = clima_filtrado[0]['dt'] + timedelta(seconds=float(t_actual_sec))
                    self.progreso_signal.emit(f"Simulando (Paso Adaptativo) {dt_fecha.strftime('%d/%m %H:%M')}...", pct)

                # Interpola variables climáticas en t_actual_sec
                tdb_k = float(np.interp(t_actual_sec, t_epw_sec, tdb_epw))
                twb_k = float(np.interp(t_actual_sec, t_epw_sec, twb_epw))
                patm_k = float(np.interp(t_actual_sec, t_epw_sec, patm_epw))

                velocidad_pct = pid.calcular(T_set, T_piscina, dt_adapt)
                m_a_actual = m_a_nom * (velocidad_pct / 100.0)

                u_ratio = velocidad_pct / 100.0
                p_elec_instantanea_kw = (p_motor_nom_kw / eta_glob) * (u_ratio ** 3)
                energia_acum_kwh += p_elec_instantanea_kw * (dt_adapt / 3600.0)

                w_sat_wb, _ = lut.get_ws_hs(twb_k)
                w_a_in = ((h_fg0_def - (cp_w_def - cp_v_def) * twb_k) * w_sat_wb - cp_a_def * (tdb_k - twb_k)) / (h_fg0_def + cp_v_def * twb_k - cp_w_def * twb_k)
                h_a_in = cp_a_def * tdb_k + w_a_in * (h_fg0_def + cp_v_def * tdb_k)

                T_salida_inst, evap_kg_s, _, _, Matriz_T_a, Matriz_niebla = simular_torre_2d_matriz(
                    NTU_ref, T_w_in_dinamica, m_w_nom, h_a_in, w_a_in, m_a_actual, patm_k,
                    Nx=6, Ny=6, lut=lut
                )
                ...
                fraccion_renovacion = min(1.0, dt_adapt / tau_sec)
                ...
                agua_acum_m3 += evap_m3h * (dt_adapt / 3600.0)

                Q_MWt = (m_w_nom * cp_w_def * delta_T_proceso) / 1000.0
                energia_disipada_mwh_t += Q_MWt * (dt_adapt / 3600.0)

                dt_fecha = clima_filtrado[0]['dt'] + timedelta(seconds=float(t_actual_sec))
                times.append(dt_fecha)
                ...
                # ADAPTACIÓN DINÁMICA DEL PASO Δt PARA EL SIGUIENTE PASO
                dT_dt = abs(T_piscina - T_piscina_prev)
                du_dt = abs(velocidad_pct - u_prev)
                
                if dT_dt > 0.3 or du_dt > 5.0:
                    dt_adapt = max(dt_min, dt_adapt * 0.7)  # Reducir paso en transitorios
                elif dT_dt < 0.05 and du_dt < 0.5:
                    dt_adapt = min(dt_max, dt_adapt * 1.3)  # Ampliar paso en régimen estacionario
                else:
                    dt_adapt = 300.0

                u_prev = velocidad_pct
                T_piscina_prev = T_piscina
                t_actual_sec += dt_adapt
            ...
            intensidad_agua_m3_mwh = (agua_acum_m3 / energia_disipada_mwh_t) if energia_disipada_mwh_t > 0 else 0.0
            intensidad_agua_m3_kwhe = (agua_acum_m3 / energia_acum_kwh) if energia_acum_kwh > 0 else 0.0
...
class VentanaSimulacionDinamica(QDialog):
    def __init__(self, parent=None, datos_torre=None):
        super().__init__(parent)
        self.setWindowTitle("Simulación Dinámica Anual / Climática (.EPW) con PID, Energía y Paso Adaptativo")
    ...
    def init_ui(self):
        ...
        # 2. Rango Temporal y Estanque
        gb_tiempo = QGroupBox("2. Rango Temporal y Estanque")
        ...
        self.txt_vol_estanque.setFont(QFont("Segoe UI", 9))
        ...
        # BLOQUE COMPLETO DE KPIs EN PANEL IZQUIERDO
        gb_kpi = QGroupBox("📊 Resultados y KPIs del Período")
        ...
        self.lbl_m3_total = QLabel("Agua Evaporada: -- m³")
        self.lbl_cop = QLabel("Rendimiento (COP): -- kWh_t/kWh_e")
        self.lbl_int_agua_mwh = QLabel("Consumo Espec. Agua: -- m³/MWh_t")
        self.lbl_int_agua_kwhe = QLabel("Agua / Energía Elec: -- m³/kWh_e")

        for lbl in [self.lbl_q_disipada, self.lbl_kwh_total, self.lbl_m3_total, 
                    self.lbl_cop, self.lbl_int_agua_mwh, self.lbl_int_agua_kwhe]:
        ...
        # PANEL DERECHO: CONTROLES DE VARIABLES
    ...
    def procesar_exito(self, res):
        ...
        # ACTUALIZACIÓN DE ETIQUETAS DE KPIS
        ...
        self.lbl_m3_total.setText(f"Agua Evaporada: <b>{res['agua_total_m3']:.2f} m³</b>")
        ...
        self.lbl_int_agua_kwhe.setText(f"Agua / Energía Elec: <b>{res['intensidad_agua_m3_kwhe']:.3f} m³/kWh_e</b>")
        ...
        if ax_top is not None:
            if self.chk_niebla.isChecked() and 'niebla' in self.res_sim:
                ...
                if np.any(niebla_mask):
                    ax_top.fill_between(
                        times, ax_top.get_ylim()[0], ax_top.get_ylim()[1],
                        where=niebla_mask, color='#7F8C8D', alpha=0.25,
                        label='Pluma/Niebla Activa', transform=ax_top.get_xaxis_transform()
                    )
