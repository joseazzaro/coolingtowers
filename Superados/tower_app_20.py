...
class DialogoPerfilPluma(QDialog):
    def __init__(self, parent=None, datos_sim=None):
        super().__init__(parent)
        self.setWindowTitle("Perfil Atmosférico de Pluma y Dispersión Hora a Hora (Briggs 2D)")
    ...
    def init_ui(self):
        ...
        # Encabezado
        lbl_info = QLabel("Análisis Dinámico de Dispersión de Pluma Atmosférica y Riesgo de Recirculación")
        lbl_info.setFont(QFont("Segoe UI", 10, QFont.Bold))
        lbl_info.setStyleSheet("color: #2C3E50;")
        layout.addWidget(lbl_info)

        # --- AQUI SE AGREGA LA SECCIÓN VISUAL DE GEOMETRÍA DE LA TORRE ---
        gb_geom = QGroupBox("Parámetros Geométricos de la Estructura")
        ...
        lbl_d = QLabel("Diámetro Boca Ventilador:")
        lbl_d.setFont(QFont("Segoe UI", 8))
        self.txt_diametro_boca = QLineEdit("3.6")
        ...
        lbl_h = QLabel("Altura Estructura Torre:")
        lbl_h.setFont(QFont("Segoe UI", 8))
        self.txt_altura_torre = QLineEdit("10.0")
        ...
        # KPIs del Instante
        self.lbl_kpis_pluma = QLabel("Seleccione una hora para evaluar el perfil...")
        ...
        gb_control = QGroupBox("Navegación Temporal en el Período Simulado")
        layout_ctrl = QHBoxLayout(gb_control)

        self.lbl_fecha_actual = QLabel("Fecha/Hora: --/--/---- --:--")
        ...
        self.btn_worst_case = QPushButton("📍 Ir a Máxima Pluma")
    ...
    def actualizar_instante(self, idx):
        ...
        self.lbl_fecha_actual.setText(f"📅 {dt_instante.strftime('%d/%m/%Y %H:%M')}")
        ...
        ax.add_patch(matplotlib.patches.Rectangle((-D_boca / 2.0, 0), D_boca, H_torre, color='#34495E', alpha=0.85, label='Torre de Enfriamiento'))
        ax.plot([-D_boca / 2.0, -D_boca / 2.0], [H_torre, H_torre + 1.0], color='#1A252F', linewidth=2.5)
        ax.plot([D_boca / 2.0, D_boca / 2.0], [H_torre, H_torre + 1.0], color='#1A252F', linewidth=2.5)
        if len(x_vis) > 0:
            ax.fill_between(x_vis, z_bot_vis, z_top_vis, color='#95A5A6', alpha=0.55, label='Pluma Visible (Saturación/Niebla)')
            ax.plot(x_vis, z_centron[es_visible], color='#7F8C8D', linestyle='--', linewidth=1.5, label='Eje Central')

        ax.plot(x_vec[~es_visible], z_centron[~es_visible], color='#3498DB', linestyle=':', alpha=0.4, label='Eje de Dispersión Térmica')

        ax.annotate('', xy=(20, H_torre + 12), xytext=(2, H_torre + 12),
                    arrowprops=dict(facecolor='#C0392B', edgecolor='#C0392B', arrowstyle='->', lw=2))
        ax.text(8, H_torre + 13.5, f"Viento Inst.: {u_wind:.1f} m/s", color='#C0392B', fontsize=8, fontweight='bold')

        ax.set_xlim(-10, 150)
        ax.set_ylim(0, max(45, H_max_vis + 8))
        ax.set_xlabel("Distancia Horizontal en Dirección del Viento (m)", fontsize=9)
        ax.set_ylabel("Altura sobre el Suelo (m)", fontsize=9)
        ax.set_title(f"Instante: {dt_instante.strftime('%d/%m/%Y %H:%M')} | v0={w_salida_m_s:.1f} m/s | T_salida={T_a_out:.1f}°C | T_amb={T_db_amb:.1f}°C", fontsize=9, fontweight='bold')
        ...
        if u_wind > 5.5 and vel_fan_pct < 35.0:
            downwash_risk = "<b style='color:#C0392B;'>CRÍTICO (Viento vence el tiro del ventilador)</b>"
        elif u_wind > 4.0:
            downwash_risk = "<b style='color:#E67E22;'>MODERADO (Deflexión severa)</b>"
        else:
            downwash_risk = "<b style='color:#27AE60;'>BAJO (Flotabilidad térmica estable)</b>"

        self.lbl_kpis_pluma.setText(
            f"<b>Velocidad Descarga (v0):</b> {w_salida_m_s:.1f} m/s | "
            f"<b>Longitud Pluma Visible:</b> {L_pluma_vis:.1f} m | "
            f"<b>Altura Máxima:</b> {H_max_vis:.1f} m | "
            f"<b>Riesgo Recirculación:</b> {downwash_risk}"
        )
